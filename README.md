

```markdown
<!-- 🎭 Welcome to my digital crib!  Grab some coffee ☕ and enjoy the ride -->

<div align="center">
  
# 👋 Yo, I'm **Vanszs**! 
### *Fullstack AI Developer - I Build Everything with AI* 🤖

<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&weight=600&size=22&pause=1000&color=00D9FF&center=true&vCenter=true&multiline=true&repeat=true&width=600&height=100&lines=%F0%9F%A4%96+Give+me+AI%2C+I'll+build+ANYTHING;%F0%9F%9A%80+Web%3F+Mobile%3F+Blockchain%3F+IoT%3F+YES.;%F0%9F%92%AA+Not+the+smartest%2C+but+the+most+adaptable;%E2%9A%A1+AI+is+my+superpower%2C+coffee+is+my+fuel" alt="Typing SVG" />

<br>

![Profile Views](https://komarev.com/ghpvc/?username=vanszs&color=00d9ff&style=for-the-badge&label=PROFILE+STALKERS)
[![GitHub followers](https://img.shields.io/github/followers/vanszs?label=Fans&style=for-the-badge&color=00d9ff)](https://github.com/vanszs)
[![GitHub User's stars](https://img.shields.io/github/stars/vanszs?affiliations=OWNER&style=for-the-badge&color=ffd700&label=Stars)](https://github.com/vanszs)

<br>

```
╔════════════════════════════════════════════════════════════════════════════╗
║  🗣️ "I'm not that smart, but with AI, I can be as good as anyone!"        ║
║                                                                            ║
║     Web?  ✅  Mobile? ✅  Blockchain? ✅  IoT? ✅  Trading Bot? ✅  AI/ML? ✅   ║
║     Speech AI? ✅  Network Security? ✅  Linux Server?  ✅  DevOps? ✅        ║
║                                                                            ║
║              Give me a problem + AI = Consider it SOLVED 🦅                ║
╚════════════════════════════════════════════════════════════════════════════╝
```

</div>

---

<div align="center">

## 🎪 **The Quick Lowdown**

| 🔥 What I Am | 🤖 My Weapon | 🐧 My OS | 📍 Base | 💼 Status |
|:---:|:---:|:---:|:---:|:---:|
| Fullstack AI Developer | AI + Determination | Arch Linux BTW | 🇮🇩 Indonesia | Open for Chaos! |

</div>

---

## 🎯 **Who Am I? (The Real Talk)**

I'm not the smartest developer out there. But with **AI as my partner**, I can tackle **anything** - from web development to smart contracts, from mobile apps to trading bots, from IoT systems to machine learning, from speech AI to server hardening.

> **"Fullstack" doesn't just mean frontend + backend. Fullstack means FULL STACK - every layer of technology, every domain, every platform. And AI is the key that unlocks all those doors.** 🔑

**🧠 My Philosophy:**
```
const vanszs = {
  title: "Fullstack AI Developer",
  philosophy: "I don't know everything, but with AI, I can LEARN everything",
  
  // What I can build (with AI assistance):
  domains: {
    web: ["Next.js", "React", "Full websites & dashboards", "Landing pages"],
    mobile: ["Flutter", "Cross-platform apps"],
    blockchain: ["Solidity", "Smart Contracts", "DeFi", "Web3"],
    iot: ["Arduino", "Raspberry Pi", "Hardware integration"],
    trading: ["Automated bots", "Multi-exchange systems"],
    ai_ml: ["NLP", "Computer Vision", "LLM Integration", "Speech AI"],
    automation: ["Bots", "Scrapers", "Workflow automation"],
    security: ["Network analysis", "Penetration testing", "Server hardening"],
    voice_ai: ["Speech-to-Speech", "Speaker Diarization", "Real-time transcription"],
    linux: ["Arch Linux", "System administration", "Shell scripting"],
    devops: ["CI/CD", "GitHub Actions", "Docker", "Server management"],
  },
  
  favoriteOS: "Arch Linux BTW 🐧",
  secret: "AI handles the 'how', I focus on the 'what' and 'why'",
  
  currentMood: "Debugging something that worked yesterday 🤔",
  coffeeLevel: "CRITICAL ☕☕☕",
};
```

**🎯 The Point Is:**
- ❌ Not a jack of all trades, master of none
- ✅ **Jack of all trades, MASTER OF AI-ASSISTED DEVELOPMENT**
- 🤖 AI = Force multiplier. Me + AI = Unstoppable.

---

## 🔥 **What Makes Me Different**

<div align="center">

| Traditional Dev | Me (Fullstack AI Dev) |
|:---:|:---:|
| Specialized in 1-2 areas | Can tackle ANY domain |
| Years to learn new stack | Days/weeks with AI assistance |
| Limited by personal knowledge | Limited only by imagination |
| "That's not my expertise" | "Give me a day" 💪 |

</div>

**🚀 Recent Proof:**
- Built a complete **Smart City IoT ecosystem** (Web + 2 Mobile Apps + Hardware) in **2 weeks**
- Developed **political party website** for government institution with Redis workers
- Created **blockchain smart contracts** without prior Solidity experience
- Developed **Speech-to-Speech AI** with speaker diarization
- Created **network security tools** for penetration testing
- Managing **Linux servers** with proper hardening and CI/CD pipelines
- Developed **trading bots** for multiple exchanges

*All with AI assistance. All delivered. All working.* ✅

---

## 🐧 **Linux & DevOps Enthusiast**

<div align="center">

![Arch Linux](https://img.shields.io/badge/Arch_Linux-1793D1?style=for-the-badge&logo=arch-linux&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=for-the-badge&logo=github-actions&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=for-the-badge&logo=gnu-bash&logoColor=white)

</div>

> **"I use Arch, BTW"** - Yes, I'm one of those people. 😎

**🔧 What I Do:**
| Area | Skills |
|------|--------|
| 🐧 **Linux** | Arch Linux daily driver, system customization, dotfiles management |
| 🔐 **Server Hardening** | Security configurations, firewall setup, SSH hardening |
| 🔄 **CI/CD** | GitHub Actions workflows, automated deployments |
| 🐳 **Containerization** | Docker, container orchestration |
| 📜 **Scripting** | Bash, automation scripts, system maintenance |
| 🖥️ **Server Management** | VPS setup, monitoring, performance optimization |

**💡 Why Arch Linux?**
- Full control over the system
- Rolling release = always up to date
- Learn Linux from the ground up
- Minimal bloat, maximum performance
- The AUR is amazing
- *And yes, I can install it without the archinstall script* 😏

---

## 🌟 **Featured Projects (Proof of Concept)**

### 🏛️ **Simokerto PINTAR - Smart City IoT Ecosystem**
*Complete government digital transformation - Web, Mobile, IoT, all in one* 🚨

> Proof that "Fullstack" isn't just web. This includes a **web dashboard**, **2 mobile apps**, and **hardware IoT integration** - all in one ecosystem.

<div align="center">

[![Web Dashboard](https://img.shields.io/badge/🏛️_Web_Dashboard-Next.js_|_TypeScript-0070f3?style=for-the-badge)](https://github.com/Vanszs/Luaran_Website_Keluarahan_Simokerto)
[![Citizens App](https://img.shields.io/badge/👥_Citizens_App-Flutter_|_Dart-02569B?style=for-the-badge&logo=flutter&logoColor=white)](https://github.com/Vanszs/Flutter_Aplikasi_Pintar_Kelurahan)
[![Officers App](https://img.shields.io/badge/👮_Officers_App-Flutter_|_Dart-02569B?style=for-the-badge&logo=flutter&logoColor=white)](https://github.com/Vanszs/Aplikasi_Pintar_Petugas)

</div>

<details>
<summary>🔥 <b>Click to see the complete breakdown!</b></summary>
<br>

**🎯 Why This Project Matters:**
This isn't just a website. This is a complete ecosystem spanning **multiple platforms and technologies**:

| Layer | Technology | What It Does |
|-------|------------|--------------|
| 🌐 Web | Next.js 15, TypeScript, Material-UI | Admin dashboard, analytics, IoT control |
| 📱 Mobile (Citizens) | Flutter, Riverpod | Report issues, track status, emergency alerts |
| 📱 Mobile (Officers) | Flutter, Firebase | Receive tasks, real-time updates, siren control |
| 🔗 IoT | WebSocket, Custom APIs | Siren activation, sensor monitoring |
| 🗄️ Backend | MySQL, Firebase FCM | Data storage, push notifications |

**🚨 IoT Innovation Highlights:**
- ⚡ **Instant Emergency Reporting** with automated IoT siren activation
- 📍 **Real-time Location Tracking** for emergency response teams
- 🔔 **Smart Alert System** with escalation protocols
- 🔧 **Hardware Integration** for government infrastructure

**📊 Impact:**
| Metric | Achievement |
|--------|-------------|
| Development Time | 2 Weeks 🚀 |
| Platforms Covered | Web + Android + iOS + IoT |
| Security Level | Enterprise-grade 🔒 |
| Response Time Reduction | 60% faster ⚡ |

**🛠️ Technical Architecture:**
```
🌐 Web Dashboard         📱 Mobile Apps            🔗 IoT Integration
├── Next.js 15           ├── Flutter 3.0+          ├── WebSocket
├── TypeScript           ├── Riverpod              ├── Firebase FCM
├── Material-UI          ├── Material Design 3     ├── IoT Siren API
├── MySQL 8.0            ├── Shared Preferences    ├── Real-time Sync
└── bcrypt Security      └── Offline Support       └── Hardware Control
```

</details>

---

### 🏛️ **NasDem Sidoarjo - Political Party Website**
*Professional government/political institution website with modern architecture* 🏢

> **Live Production**: [nasdemsidoarjo.com](https://nasdemsidoarjo.com) - Real client, real impact, real responsibility.

<div align="center">

[![NasDem Website](https://img.shields.io/badge/🏛️_NasDem_Sidoarjo-Live_Production-0070f3?style=for-the-badge)](https://nasdemsidoarjo.com)
![Private Repo](https://img.shields.io/badge/🔒_Repository-Private_(Government)-gray?style=for-the-badge)

</div>

<details>
<summary>🏢 <b>See the enterprise architecture!</b></summary>
<br>

**🎯 Project Overview:**
Freelance project for a government/political institution. Repository is private due to security and data confidentiality requirements.

**🛠️ Tech Stack:**
| Component | Technology | Purpose |
|-----------|------------|---------|
| 🌐 Frontend | Next.js, TypeScript | Landing page + Admin dashboard |
| ⚙️ Backend | Node.js, REST API | Business logic & data management |
| 🗄️ Database | MySQL/PostgreSQL | Data persistence |
| 🔄 Queue/Worker | Redis | Background jobs, caching, task queue |
| 🔐 Security | Enterprise-grade auth | Government-level security standards |

**📊 Features:**
- ✅ **Public Landing Page** - Professional party information & news
- ✅ **Admin Dashboard** - Content management system
- ✅ **Redis Workers** - Background processing & job queues
- ✅ **Real-time Updates** - Live content synchronization
- ✅ **SEO Optimized** - High search engine visibility
- ✅ **Government Compliance** - Security & data protection standards

**💼 Client Impact:**
- Professional online presence for political institution
- Streamlined content management for staff
- High-performance architecture with Redis caching
- Secure handling of sensitive political data

</details>

---

### 🎤 **Speech-to-Speech Diarization**
*Real-time voice AI with speaker identification* 🗣️

> Cutting-edge speech AI - can distinguish between multiple speakers in a single audio stream.

<div align="center">

[![Speech AI](https://img.shields.io/badge/🎤_Speech_Diarization-Python_|_AI-9333ea?style=for-the-badge)](https://github.com/Vanszs/Speach-To-Speach-Diarization)

</div>

<details>
<summary>🧠 <b>Explore the voice AI magic!</b></summary>
<br>

**🎯 What It Does:**
| Feature | Description |
|---------|-------------|
| 🎤 Speech-to-Text | Real-time transcription |
| 👥 Speaker Diarization | Identify WHO is speaking |
| 🔄 Speech-to-Speech | Voice transformation pipeline |
| ⚡ Real-time Processing | Live audio stream handling |

**🛠️ Tech Stack:**
- **Language**: Python
- **AI/ML**: Speech recognition models, diarization algorithms
- **Audio Processing**: Real-time stream handling
- **Use Cases**: Meeting transcription, interview analysis, call center AI

**💡 Innovation:**
This isn't just transcription - it's **understanding conversations** by identifying different speakers automatically.

</details>

---

### 🤖 **Arcalis - Decentralized AI Ecosystem**
*Privacy-first AI platform with blockchain governance* 🛡️

> Combining **AI**, **Blockchain**, and **Web3** in one platform. This isn't one domain - it's three domains at once.

[![Arcalis AI](https://img.shields.io/badge/🤖_Arcalis_AI-AI_+_Blockchain_+_Web3-9333ea?style=for-the-badge)](https://github.com/Vanszs/Manta-Hackaton-v1.0)

<details>
<summary>🧠 <b>Expand to see the multi-domain magic!</b></summary>
<br>

**🎯 Domains Combined:**
| Domain | Implementation |
|--------|----------------|
| 🤖 AI/ML | 6+ AI models (DeepSeek R1, Gemini 2.0, Phi-4) |
| ⛓️ Blockchain | Manta Network, Solidity Smart Contracts |
| 🌐 Web3 | DAO governance, on-chain voting |
| 🔐 Privacy | Zero-Knowledge Proofs |
| 💰 DeFi | Treasury management, token economics |

**🚀 Innovation Highlights:**
- **Multi-model AI orchestration** - not just one AI, but 6+
- **90% cheaper** than traditional AI platforms
- **Carbon-neutral** infrastructure
- **Quadratic voting** for fair governance

</details>

---

### ⛓️ **BlockTix - Blockchain Event Management**
*Smart contracts for transparent ticketing* 🎫

> Zero Solidity experience before. With AI, delivered in weeks.

[![BlockTix](https://img.shields.io/badge/🎫_BlockTix-Solidity_|_Web3-f97316?style=for-the-badge)](https://github.com/Vanszs/ICP-BlockTix-Contract)

<details>
<summary>🎟️ <b>See the blockchain journey!</b></summary>
<br>

**🔗 What I Built (First Time in Solidity!):**
- 🔒 **Smart Contract Security**: Event management on-chain
- 🔗 **Multi-network**: Ethereum Holesky & Bitfinity deployment
- ✨ **Transparent Ticketing**: No more scalpers
- 💱 **ETH-USD Conversion**: Simplified payments

**📈 Stats:**
- **17 Commits** of learning + building
- **Multiple Networks** ready
- **React + Web3** frontend integration

</details>

---

### 🛡️ **Network Security Tools**
*Penetration testing & network analysis* 🔐

> Tools for network security research and ethical hacking.

<div align="center">

[![MikuMikuBeam](https://img.shields.io/badge/⚡_MikuMikuBeam-Network_Stress_Testing-ff6b6b?style=for-the-badge)](https://github.com/Vanszs/MikuMikuBeamDDOS)
[![Evillimiter](https://img.shields.io/badge/🔍_Evillimiter-Bandwidth_Analysis-purple?style=for-the-badge)](https://github.com/Vanszs/Evillimiter)

</div>

<details>
<summary>🔐 <b>Explore the security toolkit!</b></summary>
<br>

**⚡ MikuMikuBeam DDoS** (17⭐)
| Info | Details |
|------|---------|
| 🎯 Purpose | Network stress testing tool |
| 🛠️ Tech | TypeScript |
| 📝 License | MIT |
| ⭐ Stars | 17 |
| 🍴 Forks | 2 |

> An open-source network stresser tool but it's Hatsune Miku themed! 🎤

**🔍 Evillimiter**
| Info | Details |
|------|---------|
| 🎯 Purpose | Monitor, analyze & limit bandwidth on local network |
| 🛠️ Tech | Python |
| 📝 License | MIT |
| 🔧 Feature | Works without admin access |

> Tool for network analysis - monitor and manage device bandwidth on local networks.

**⚠️ Disclaimer:** These tools are for **educational purposes** and **authorized testing only**. Always get proper permission before testing any network.

</details>

---

### 💰 **Trading Bots & Financial Tech**
*Automated trading across multiple exchanges* 📈

> No finance background. Not a quant. But with AI, can build production-ready trading systems.

<div align="center">

[![Crypto API](https://img.shields.io/badge/📈_Multi_Exchange_API-Indodax_|_Bybit-00d4aa?style=for-the-badge)](https://github.com/Vanszs/TugasPemrogrammanCrypto_IndodaxBybitAPI)
[![AutoTrade](https://img.shields.io/badge/🚀_AutoTrade_Bot-Risk_Management-ff6b6b?style=for-the-badge)](https://github.com/Vanszs/Bybit_AutoTrade)

</div>

---

### 🤖 **AI/ML Projects**
*From NLP to Computer Vision to Speech AI* 🧠

> Machine learning isn't just theory. These are real implementations.

<div align="center">

[![Sentiment](https://img.shields.io/badge/🧠_Transformer_NLP-Sentiment_Analysis-purple?style=for-the-badge)](https://github.com/Vanszs/Transformer_Sentiment_Analyze)
[![YOLO](https://img.shields.io/badge/🚗_YOLO_Vision-Autonomous_Systems-green?style=for-the-badge)](https://github.com/Vanszs/yolo-autonomus)
[![Evolution](https://img.shields.io/badge/🧬_Evolution_Strategies-Genetic_Algorithms-blue?style=for-the-badge)](https://github.com/Vanszs/Evolution_Strategies)
[![Speech](https://img.shields.io/badge/🎤_Speech_AI-Diarization-orange?style=for-the-badge)](https://github.com/Vanszs/Speach-To-Speach-Diarization)

</div>

---

## 🛠️ **The Complete Arsenal**

<div align="center">

### 💻 **Languages**
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![Dart](https://img.shields.io/badge/Dart-0175C2?style=for-the-badge&logo=dart&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Solidity](https://img.shields.io/badge/Solidity-363636?style=for-the-badge&logo=solidity&logoColor=white)
![C++](https://img.shields.io/badge/C++-00599C?style=for-the-badge&logo=cplusplus&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=for-the-badge&logo=gnu-bash&logoColor=white)

### 🌐 **Web Development**
![Next.js](https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=nextdotjs&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Node.js](https://img.shields.io/badge/Node.js-43853D?style=for-the-badge&logo=node.js&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white)
![Material-UI](https://img.shields.io/badge/MUI-0081CB?style=for-the-badge&logo=material-ui&logoColor=white)

### 📱 **Mobile Development**
![Flutter](https://img.shields.io/badge/Flutter-02569B?style=for-the-badge&logo=flutter&logoColor=white)
![Dart](https://img.shields.io/badge/Dart-0175C2?style=for-the-badge&logo=dart&logoColor=white)

### ⛓️ **Blockchain & Web3**
![Ethereum](https://img.shields.io/badge/Ethereum-3C3C3D?style=for-the-badge&logo=Ethereum&logoColor=white)
![Solidity](https://img.shields.io/badge/Solidity-363636?style=for-the-badge&logo=solidity&logoColor=white)
![Web3.js](https://img.shields.io/badge/Web3.js-F16822?style=for-the-badge&logo=web3.js&logoColor=white)

### 🔌 **IoT & Hardware**
![Arduino](https://img.shields.io/badge/Arduino-00979D?style=for-the-badge&logo=Arduino&logoColor=white)
![Raspberry Pi](https://img.shields.io/badge/Raspberry%20Pi-A22846?style=for-the-badge&logo=Raspberry%20Pi&logoColor=white)
![MQTT](https://img.shields.io/badge/MQTT-660066?style=for-the-badge&logo=eclipse-mosquitto&logoColor=white)

### 🤖 **AI & Automation**
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![TensorFlow](https://img.shields.io/badge/TensorFlow-FF6F00?style=for-the-badge&logo=tensorflow&logoColor=white)
![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=for-the-badge&logo=openai&logoColor=white)

### 🐧 **Linux & System Administration**
![Arch Linux](https://img.shields.io/badge/Arch_Linux-1793D1?style=for-the-badge&logo=arch-linux&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![Ubuntu](https://img.shields.io/badge/Ubuntu-E95420?style=for-the-badge&logo=ubuntu&logoColor=white)
![Shell Script](https://img.shields.io/badge/Shell_Script-121011?style=for-the-badge&logo=gnu-bash&logoColor=white)

### 🗄️ **Database & Backend**
![MySQL](https://img.shields.io/badge/MySQL-005C84?style=for-the-badge&logo=mysql&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-DC382D?style=for-the-badge&logo=redis&logoColor=white)
![Firebase](https://img.shields.io/badge/Firebase-039BE5?style=for-the-badge&logo=Firebase&logoColor=white)
![Socket.IO](https://img.shields.io/badge/Socket.io-black?style=for-the-badge&logo=socket.io&badgeColor=010101)

### 🔧 **DevOps & Tools**
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=for-the-badge&logo=github-actions&logoColor=white)
![Nginx](https://img.shields.io/badge/Nginx-009639?style=for-the-badge&logo=nginx&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-0078D4?style=for-the-badge&logo=visual%20studio%20code&logoColor=white)
![Neovim](https://img.shields.io/badge/Neovim-57A143?style=for-the-badge&logo=neovim&logoColor=white)

</div>

---

## 📊 **The Numbers**

<div align="center">

<img src="https://github-readme-stats.vercel.app/api?username=vanszs&show_icons=true&theme=radical&hide_border=true&count_private=true&bg_color=0d1117&title_color=00d9ff&icon_color=00d9ff&text_color=ffffff" height="180"/>
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=vanszs&layout=compact&theme=radical&hide_border=true&langs_count=8&bg_color=0d1117&title_color=00d9ff&text_color=ffffff" height="180"/>

<br>

![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=vanszs&theme=radical&hide_border=true&background=0d1117&ring=00d9ff&fire=ff6b6b&currStreakLabel=00d9ff)

<br>

![GitHub Profile Trophy](https://github-profile-trophy.vercel.app/?username=vanszs&theme=radical&no-frame=true&row=1&column=7&margin-w=15)

</div>

---

## 🗂️ **Complete Project Portfolio**

<details>
<summary><b>🏛️ Government & Political Tech (2024-2025)</b></summary>

| Project | Stack | Description |
|---------|-------|-------------|
| [🌐 Simokerto Web](https://github.com/Vanszs/Luaran_Website_Keluarahan_Simokerto) | Next.js, TypeScript | Admin dashboard + IoT control |
| [📱 Citizen App](https://github.com/Vanszs/Flutter_Aplikasi_Pintar_Kelurahan) | Flutter | Emergency reporting + offline sync |
| [👮 Officer App](https://github.com/Vanszs/Aplikasi_Pintar_Petugas) | Flutter | Real-time tasks + siren control |
| 🏛️ [NasDem Sidoarjo](https://nasdemsidoarjo.com) | Next.js, Redis | Political party website (Private) |
| [🔄 NasDem V2.0](https://github.com/Vanszs/Nasdem_Web_V2.0) | TypeScript | Enhanced version with Redis workers |

</details>

<details>
<summary><b>💰 Trading & FinTech (2024-2025)</b></summary>

| Project | Stack | Description |
|---------|-------|-------------|
| [📈 Crypto API](https://github.com/Vanszs/TugasPemrogrammanCrypto_IndodaxBybitAPI) | TypeScript | Multi-exchange automation |
| [🚀 AutoTrade Bot](https://github.com/Vanszs/Bybit_AutoTrade) | Python | Risk-managed trading |
| [💰 XAUUSD Pred Bot](https://github.com/Vanszs/XAUUSD_Pred_Bot) | Python | Gold price prediction |

</details>

<details>
<summary><b>🤖 AI/ML & Voice Tech (2024-2025)</b></summary>

| Project | Stack | Description |
|---------|-------|-------------|
| [🎤 Speech Diarization](https://github.com/Vanszs/Speach-To-Speach-Diarization) | Python | Speech-to-Speech with speaker ID |
| [🧠 Transformer NLP](https://github.com/Vanszs/Transformer_Sentiment_Analyze) | Python | Sentiment analysis |
| [🧬 Evolution Strategies](https://github.com/Vanszs/Evolution_Strategies) | Python | Genetic algorithms |
| [🚗 YOLO Autonomous](https://github.com/Vanszs/yolo-autonomus) | Python | Computer vision |
| [📋 Logbook Automation](https://github.com/Vanszs/Silaturahmi-Logbook-Automation) | Python | Workflow automation |

</details>

<details>
<summary><b>🛡️ Security & Network Tools (2024-2025)</b></summary>

| Project | Stack | Description |
|---------|-------|-------------|
| [⚡ MikuMikuBeam](https://github.com/Vanszs/MikuMikuBeamDDOS) | TypeScript | Network stress testing (17⭐) |
| [🔍 Evillimiter](https://github.com/Vanszs/Evillimiter) | Python | Bandwidth monitoring & limiting |
| [🔒 Free Proxy](https://github.com/Vanszs/Free-Proxy) | - | Proxy management |

</details>

<details>
<summary><b>⛓️ Blockchain & Web3 (2024)</b></summary>

| Project | Stack | Description |
|---------|-------|-------------|
| [🎫 BlockTix](https://github.com/Vanszs/ICP-BlockTix-Contract) | Solidity | Smart contract ticketing |
| [🏆 Arcalis/Manta](https://github.com/Vanszs/Manta-Hackaton-v1.0) | TypeScript, Web3 | Decentralized AI platform |
| [📊 D.A.T.A](https://github.com/Vanszs/d.a.t.a) | - | AI Agent authentication |
| [🔧 Web3 Scripts](https://github.com/Vanszs/web3-scripts) | Python | Blockchain automation |
| [⚡ MEV Toolkit](https://github.com/Vanszs/mev-toolkit) | Shell | MEV operations |

</details>

<details>
<summary><b>🤖 Automation Bots (2024)</b></summary>

| Project | Description |
|---------|-------------|
| [💰 HotWallet Bot](https://github.com/Vanszs/HotWalletBot) | Web3 wallet automation |
| [🌊 Wave Bot](https://github.com/Vanszs/Wave-Bot) | Airdrop automation |
| [🚀 Omni Bot](https://github.com/Vanszs/OnmiBot) | Multi-chain accounts |
| [😂 Meme Bot](https://github.com/Vanszs/MemeBot) | Memefi automation |
| [🎯 Va Bot](https://github.com/Vanszs/Va) | WhatsApp automation |

</details>

<details>
<summary><b>💼 Professional Work (2024)</b></summary>

| Project | Stack | Description |
|---------|-------|-------------|
| [🏢 IGS Web](https://github.com/Vanszs/Magang_Web_IGS) | TypeScript | Enterprise web app |
| [📱 IGS Mobile](https://github.com/Vanszs/Flutter_Project_IGS_Magang) | Flutter | Cross-platform app |
| [🛒 E-commerce](https://github.com/Vanszs/MINIMAL-ECOMERCE-TALL-STACK) | Laravel, Livewire | TALL stack shop |

</details>

<details>
<summary><b>🔍 Data & Scraping (2024-2025)</b></summary>

| Project | Description |
|---------|-------------|
| [🔧 Toolify Scraper](https://github.com/Vanszs/toolify_scrap) | Account data scraping |
| [📁 Collab Archive](https://github.com/Vanszs/collab-archive) | Team collaboration |

</details>

<details>
<summary><b>🌐 Web & Design (2024)</b></summary>

| Project | Description |
|---------|-------------|
| [🌊 Nautical Nusantara](https://github.com/Vanszs/NauticalNusantara) | Competition site |
| [🌸 Flower HTML](https://github.com/Vanszs/Flower-html) | CSS animations |
| [🎨 Kadoo](https://github.com/Vanszs/kadoo) | UI components |
| [❄️ Login Yeti](https://github.com/Vanszs/Login-yeti) | Creative login |
| [📱 Portfolio](https://github.com/Vanszs/Vanszs.github.io) | Personal site |

</details>

---

## 🏆 **Achievements Unlocked**

<div align="center">

```
╔═══════════════════════════════════════════════════════════════════════════╗
║                        🎖️ FULLSTACK AI DEV BADGES 🎖️                      ║
╠═══════════════════════════════════════════════════════════════════════════╣
║                                                                           ║
║  🏛️ SMART CITY ARCHITECT    Built complete govt ecosystem in 2 weeks    ║
║  🏢 POLITICAL TECH          Live production site for govt institution   ║
║  ⛓️ BLOCKCHAIN PIONEER       First Solidity project? Deployed & working  ║
║  🤖 AI ORCHESTRATOR          6+ AI models in one platform               ║
║  🎤 VOICE AI ENGINEER        Speech-to-Speech with diarization          ║
║  📱 MOBILE MASTER            Cross-platform apps that actually work      ║
║  🔌 IOT COMMANDER            Software meets hardware magic               ║
║  🛡️ SECURITY RESEARCHER      Network analysis & penetration tools       ║
║  🐧 LINUX ENTHUSIAST         Arch Linux BTW - server hardening expert   ║
║  🔧 DEVOPS PRACTITIONER      CI/CD, GitHub Actions, Docker              ║
║  🔐 SECURITY GUARDIAN        Enterprise-grade auth everywhere            ║
║  🧠 ML PRACTITIONER          NLP, CV, Genetic Algorithms, Speech AI     ║
║  🌐 OPEN SOURCE HERO         58+ repos, full documentation              ║
║                                                                           ║
╚═══════════════════════════════════════════════════════════════════════════╝
```

</div>

---

## 🔮 **2025 Roadmap**

**🎯 The Mission Continues:**
- 🤖 **More AI Integration** - Expanding into more AI-assisted domains
- 🎤 **Voice AI** - Advanced speech processing systems
- 🏛️ **Scale Smart City** - Taking Simokerto PINTAR nationwide
- ⛓️ **DeFi Deep Dive** - Advanced protocols and mechanisms
- 🧠 **AI Agents** - Autonomous systems that work for me
- 📱 **Flutter Ecosystem** - Publishing widget libraries
- 🐧 **Linux Mastery** - Deeper into Arch, custom kernels, system optimization
- 🔐 **Server Hardening** - Advanced security configurations
- 🔄 **DevOps Excellence** - More complex CI/CD pipelines
- 💰 **Trading Evolution** - AI-powered strategies
- 🛡️ **Security Research** - Deeper into ethical hacking
- 🌐 **Open Source** - Contributing to major projects

**💡 Philosophy for 2025:**
> "The goal isn't to know everything. The goal is to be able to BUILD anything."

---

## 🌐 **Let's Connect!**

<div align="center">

[![Portfolio](https://img.shields.io/badge/🌐_Portfolio-255E63?style=for-the-badge&logoColor=white)](https://vanszs.github.io)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/bevansatria)
[![Twitter](https://img.shields.io/badge/Twitter-1DA1F2?style=for-the-badge&logo=twitter&logoColor=white)](https://twitter.com/yourjungler)
[![Discord](https://img.shields.io/badge/Discord-7289DA?style=for-the-badge&logo=discord&logoColor=white)](https://discord.gg/belugaa99)

<br>

| 📧 Email | 🌍 Location | 🐧 OS | 💼 Status |
|:--------:|:-----------:|:-----:|:---------:|
| [bevansatria@gmail.com](mailto:bevansatria@gmail.com) | Indonesia 🇮🇩 | Arch BTW | Open for Everything! |

**💡 Got a crazy project idea? I probably can build it.**

</div>

---

## 📈 **Activity Graph**

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=Vanszs&theme=react-dark&hide_border=true&bg_color=0d1117&color=00d9ff&line=00d9ff&point=ff6b6b" />

</div>

---

## 💭 **Final Words**

<div align="center">

```
╔═══════════════════════════════════════════════════════════════════════════════════╗
║                                                                                   ║
║  "Technology should empower communities, enhance security, and create            ║
║   opportunities for everyone. Every line of code is a step toward a more         ║
║   connected, efficient, and safer world."                                         ║
║                                                                                   ║
║   I may not be the smartest developer in the room.                                ║
║   But with AI by my side, I can be the most VERSATILE.                            ║
║                                                                                   ║
║   Also, I use Arch, BTW. 🐧                                                       ║
║                                                                                   ║
║                                   - Vanszs, Fullstack AI Developer 🤖            ║
║                                                                                   ║
╚═══════════════════════════════════════════════════════════════════════════════════╝
```

</div>

**🌟 Core Values:**
| Value | What It Means |
|-------|---------------|
| 🤖 **AI-First Mindset** | Leverage AI to break limitations |
| 🐧 **Linux Philosophy** | Control your system, understand your tools |
| 💡 **Infinite Curiosity** | Every domain is learnable |
| 🚀 **Ship Fast** | Done is better than perfect |
| 🔐 **Security Always** | Never compromise on safety |
| 🤝 **Open Sharing** | Knowledge grows when shared |
| 🌍 **Real Impact** | Build things that matter |

---

<div align="center">

## 🙏 **Thanks for Reading!**

*If you made it this far, you're awesome.* 💪

**Got a project that seems impossible? Hit me up. With AI, impossible is just a longer prompt.** 😎

<br>

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=100&section=footer&text=⭐%20Star%20if%20you%20believe%20in%20AI-powered%20development!&fontSize=14&fontColor=ffffff&animation=twinkling"/>

<br>

**Made with ❤️, 🤖, 🐧, and ☕ by Vanszs**

*Fullstack AI Developer | Arch Linux Enthusiast | Indonesia 🇮🇩*

*Last Updated: December 2025*

</div>

<!-- 
  Easter Egg for fellow devs reading the source: 
  
  Q: How many developers does it take to build a complete system?
  A: Just one... if they have AI 🤖
  
  Q: What's the best Linux distro?
  A: The one you actually use. But also Arch. 🐧
  
  The future isn't about knowing everything.
  It's about being able to LEARN and BUILD anything.
  
  Stay curious, stay humble, stay building 🚀
  
  btw I use arch
-->
```
